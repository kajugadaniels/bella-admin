...
export default function InventorySnapshot({ rows = [], ccy = "RWF" }) {
    return (
        <GlassCard className="p-4">
            <SectionTitle icon={Boxes}>Inventory snapshot (top remaining)</SectionTitle>
            <div className="mt-2 rounded-xl border border-neutral-200/70 dark:border-neutral-800 overflow-hidden">
                ...
                {rows.length === 0 ? (
                    <div className="px-3 py-6 text-sm text-neutral-500">No inventory.</div>
                ) : (
                    rows.map((r) => (
                        <div key={r.id} className="grid grid-cols-12 px-3 py-2 hover:bg-black/[0.03] dark:hover:bg-white/[0.04]">
                            ...
                            <div className="col-span-2 text-right">{r.remaining}</div>
                            <div className="col-span-2 text-right">{currency(r?.product?.unit_price, ccy)}</div>
                            <div className="col-span-2 text-right font-semibold">{currency(r.value_gross, ccy)}</div>
                        </div>
                    ))
                )}
            </div>
