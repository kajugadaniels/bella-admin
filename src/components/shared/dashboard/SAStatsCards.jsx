        ...
        >
            <div className="flex items-center justify-between">
                <div>
                    ...
                    {hint ? (
                        <div className="text-[11px] mt-0.5 text-neutral-500">{hint}</div>
                    ) : null}
                </div>
    ...
    return (
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
            ...
            <StatCard
                title="Orders"
                value={ordersCount}
                hint={`${paidCount} paid • ${pendingCount} pending`}
                icon={PackageCheck}
            ...
            <StatCard
                title="Stock-in (qty)"
                value={stockinQty.toLocaleString()}
                icon={PackageOpen}
            ...
            <StatCard
                title="Stock-out (qty)"
                value={stockoutsQty.toLocaleString()}
                icon={Gauge}
            ...
            <StatCard
                title="Stock value"
                value={currency(stockValueGross)}
                hint={`Net ${currency(stockValueNet)}`}
                icon={TrendingUp}
            ...
            <StatCard
                ...
                value={`${productsPublished}/${productsTotal}`}
                icon={CheckCircle2}
